...
for parent in os.listdir(root):
    for actor in os.listdir(os.path.join(root, parent)):
        for audiofile in os.listdir(os.path.join(root, parent, actor)):
            if not audiofile.endswith('.wav') or 'croppad' in audiofile:
                continue
            audios = librosa.core.load(os.path.join(root, parent, actor, audiofile), sr=22050)
            ...
            sf.write(os.path.join(root, parent, actor, audiofile[:-4]+'_croppad.wav'), y, sr)
