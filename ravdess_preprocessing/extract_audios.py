...
for parent in os.listdir(root):
    ...
    for audiofile in os.listdir(os.path.join(root, parent)):
        if not audiofile.endswith('.wav') or 'croppad' in audiofile:
            continue
        audios = librosa.core.load(os.path.join(root, parent, audiofile), sr=22050)
        ...
        sf.write(os.path.join(root, parent, audiofile[:-4]+'_croppad.wav'), y, sr)
