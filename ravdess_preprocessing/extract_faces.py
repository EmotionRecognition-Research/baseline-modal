...
for parent in os.listdir(root):
    print(parent)
    for filename in tqdm(sorted(os.listdir(os.path.join(root, parent)))):
        # for filename in os.listdir(os.path.join(root, parent, sess)):
            
        if filename.endswith('.mp4'):

            cap = cv2.VideoCapture(os.path.join(root, parent, filename))
            ...
            if save_avi:
                out = cv2.VideoWriter(os.path.join(root, parent, filename[:-4]+'_facecroppad.avi'),cv2.VideoWriter_fourcc('M','J','P','G'), save_fps, (224,224))
            ...
            if save_avi:
                out.release()
            np.save(os.path.join(root, parent, filename[:-4]+'_facecroppad.npy'), np.array(numpy_video))
