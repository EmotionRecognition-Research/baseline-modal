...
n_processed = 0
for filename in tqdm(sorted(os.listdir(os.path.join(script_dir, root)))):    
    if filename.endswith(ext) or filename.endswith('.flv'):
        ...
        while True:
            ...
            if not i:
                break
            framen += 1
        cap = cv2.VideoCapture(os.path.join(script_dir, root, filename))
        ...
        if save_avi:
            out = cv2.VideoWriter(os.path.join(script_dir, root, filename[:-4]+'_facecroppad.avi'),cv2.VideoWriter_fourcc('M','J','P','G'), save_fps, (224,224))
        ...
        if save_avi:
            out.release() 
        np.save(os.path.join(script_dir, root, filename[:-4]+'_facecroppad.npy'), np.array(numpy_video))
        if len(numpy_video) != 15:
            print('Error', script_dir, filename)    
